...
@Test func virtualPreferenceRoundTrip() throws {
    ...
    #expect(VirtualPreference.load(from: file) == nil)          // missing → nil
    ...
    #expect(VirtualPreference.load(from: file) == nil)          // cleared → nil
}
...
@Test func virtualPreferenceRejectsInvalid() throws {
    ...
    try #"{"size": [100, 1080]}"#.data(using: .utf8)!.write(to: file)   // width out of range
    #expect(VirtualPreference.load(from: file) == nil)
    try #"{"size": [1920]}"#.data(using: .utf8)!.write(to: file)        // width only
    #expect(VirtualPreference.load(from: file) == nil)
