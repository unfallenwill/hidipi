...
from hidpi_cli import autostart, cli, state
...
class AutostartTests(unittest.TestCase):
    ...
    def test_bootstrap_failure_removes_new_plist_and_preserves_backup(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            def launch(*args, **kwargs):
                if args[0] == 'bootstrap':
                    raise cli.HiDPIError('failure')
            with patch.object(autostart, 'agent_path', return_value=path), patch.object(
                cli, 'Mac', return_value=FakeMac()), patch.object(cli.tempfile, 'gettempdir', return_value=folder), patch.object(
    ...
    def test_wait_handles_changed_display_id(self):
        ...
        self.assertTrue(autostart.wait_for_display(mac, args))

    def test_uninstall_does_not_delete_agent_if_bootout_fails(self):
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            backup = cli.write_backup(snapshot(), root)
            path = root/'agent.plist'
            path.write_bytes(plistlib.dumps(autostart.make_plist(Path('/python'), root, root,
                snapshot()['displays'][0]['uuid'], mode(), backup)))
            def launch(*args, **kwargs):
                if args[0] == 'bootout':
                    raise cli.HiDPIError('denied')
                return subprocess.CompletedProcess(args, 0, '', '')
            with patch.object(autostart, 'agent_path', return_value=path), patch.object(
                autostart, 'launchctl', side_effect=launch), self.assertRaises(cli.HiDPIError):
                autostart.uninstall(argparse.Namespace(backup_dir=root))
    ...
    def test_uninstall_restores_install_backup_and_keeps_history(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with patch.object(autostart, 'agent_path', return_value=path), patch.object(
                ...
                cli, 'Mac', return_value=mac), patch.object(cli.tempfile, 'gettempdir', return_value=folder), contextlib.redirect_stdout(io.StringIO()):
                autostart.uninstall(argparse.Namespace(backup_dir=root))
