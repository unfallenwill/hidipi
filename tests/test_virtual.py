...
from hidpi_cli import autostart, cli, virtual
...
class VirtualTests(unittest.TestCase):
    ...
    def test_fallback_is_not_replayed_as_physical_hardware(self):
        ...
        self.assertEqual(cli.validate_backup(saved), saved)
    ...
    def test_truly_empty_headless_backup_roundtrips(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            saved = virtual.capture_original(mac)
            path = cli.write_backup(saved, Path(folder))
            self.assertTrue(path.exists())

    def test_empty_legacy_backup_still_rejected(self):
        with self.assertRaises(cli.HiDPIError):
            cli.validate_backup(dict(schema=1, displays=[]))
        with self.assertRaises(cli.HiDPIError):
            cli.validate_backup(dict(schema=2, headless=False, displays=[]))
    ...
    def test_invalid_mode_sizes_and_refresh_rejected(self):
        for size, refresh in [((7680, 4320), 60), ((1920, 1080), float('nan')),
            ...
            with self.subTest(size=size, refresh=refresh), self.assertRaises(cli.HiDPIError):
    ...
    def test_failed_start_still_closes_owned_display(self):
        ...
        mac.snapshot.return_value = dict(schema=2, headless=True, displays=[])
        mac.ids.return_value = []
        display.start.side_effect = cli.HiDPIError('mode not available')
        with tempfile.TemporaryDirectory() as folder, patch.object(
            ...
            with self.assertRaisesRegex(cli.HiDPIError, 'mode not available'):
    ...
    def test_backup_failure_prevents_virtual_creation(self):
        ...
        with patch.object(cli, 'write_backup', side_effect=OSError('disk full')), patch.object(
    ...
    def test_virtual_preview_completion_removes_screen(self):
        ...
        with tempfile.TemporaryDirectory() as folder, patch.object(
                virtual, 'VirtualDisplay', return_value=display), patch.object(cli, 'preview'), contextlib.redirect_stdout(io.StringIO()):
